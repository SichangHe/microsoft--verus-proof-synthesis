...
- **Run-launch script pattern.** See [`runs/verusage_plus/vest__all22__20260510T012907Z/launch.sh`](../../runs/verusage_plus/vest__all22__20260510T012907Z/launch.sh) for the exact Claude CLI invocation, including `--print --verbose --output-format stream-json --include-partial-messages --max-budget-usd 30 --setting-sources project,local --strict-mcp-config`. The last two flags are **experimental-isolation** flags: `--setting-sources project,local` skips `~/.claude/settings.json` (so personal `effortLevel`, `permissions`, `theme` cannot bleed in), and `--strict-mcp-config` skips user-installed MCP servers (so personal Gmail/Calendar/Drive tools cannot land in the agent's tool list). They preserve Claude Code built-in agents, skills, and tools, which the user has declared in scope. `--bare` would be the comprehensive isolation flag but it blocks OAuth, which is incompatible with claude.ai/Max billing on this host. Auto-memory and CLAUDE.md auto-discovery have no per-flag disable; the relevant directories happen to be empty on this host but a future user must verify. After the agent exits, the script invokes [`tools/verusage_plus/extract_turns.py`](../../tools/verusage_plus/extract_turns.py) which writes a numbered, PWD-stripped `logs/turns.txt` next to the raw JSONL for human review. Reuse as-is for the next pilot, only changing model / cost cap / working dir.
