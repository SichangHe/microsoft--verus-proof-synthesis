...
- **Try ideas instead of debating them.** `cargo verus verify` is your oracle, and a failed verify costs ~10 s of compile time. If you have a candidate proof tactic, write it and run `cargo verus verify` rather than reasoning at length about whether it will work. Verus rejects bad ideas at almost no cost, so prefer fast empirical iteration over long deliberation.
